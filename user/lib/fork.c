...
static void duppage(u_int envid, u_int vpn) {
	...
	/* Exercise 4.10: Your code here. (2/2) */
		...
		syscall_mem_map(0, vpn * BY2PG, envid, vpn * BY2PG, new_perm);
		syscall_mem_map(0, vpn * BY2PG, 0, vpn * BY2PG, new_perm);
	}
}
...
int fork(void) {
	...
	/* Exercise 4.15: Your code here. (1/2) */
        for(int vpn = 0; vpn < USTACKTOP / BY2PG; vpn++){
		duppage(child, vpn);
	}
